...
/**
 *
 * @description 电子钱包消费打印小票模板
 * Created by kuan on 2017/FONT_SIZE_MIDDIM/29.
 */

public class EWalletConsumePrintTemplet {
    ...
    public List<PrintItemObj> printEWallet(TransactionInfo info, boolean isRePrint){
        ...
        printItemObjs.add(new PrintItemObj("交易类型/TRANS TYPE          "
//                + "有效期/EXP DATE"
                , FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj(StringUtil.getTypeName(info.getType(),info.getApptype())+"          "
//                +(null == info.getCardInfo().getExpdate()&& TextUtils.isEmpty(info.getCardInfo().getExpdate())?"未知":StringUtil.formatDate(info.getCardInfo().getExpdate()))
                , FONT_SIZE_MIDDIM, false));
        ...
        printItemObjs.add(new PrintItemObj("应用标识AID："+"485A2E4444630320000000000", FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("发卡方标识："+info.getCardInfo().getIssuerIdentifier(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("脱机/联机交易序号："+info.getCardInfo().getOfflineTransactionNo(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("秘钥版本号："+info.getCardInfo().getKeyVersionNo(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("秘钥索引号："+info.getCardInfo().getKeyIndexNo(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("终端机编号："+info.getCardInfo().getTerminalCode(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("终端交易序号："+info.getCardInfo().getPsamOfflineTransactionNo(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("TAC："+info.getCardInfo().getTac(), FONT_SIZE_SMALL, false));
        printItemObjs.add(new PrintItemObj("----------------------------------------------"));
