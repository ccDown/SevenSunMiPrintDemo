...
import com.seven.sevensunmiprintdemo.R;
import com.seven.sevensunmiprintdemo.bean.PrintItemObj;
import com.seven.sevensunmiprintdemo.bean.TableItem;
...
public class AidlUtil {
     ...
     * 打印文字带回调
     */
    public void printTextWithCallBack(String content, float size, boolean isBold, boolean isUnderLine,ICallback iCallback) {
        if (woyouService == null) {
            Toast.makeText(context, R.string.toast_2, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        try {
            if (isBold) {
                woyouService.sendRAWData(ESCUtil.boldOn(), iCallback);
            } else {
                woyouService.sendRAWData(ESCUtil.boldOff(), iCallback);
            }

            if (isUnderLine) {
                woyouService.sendRAWData(ESCUtil.underlineWithOneDotWidthOn(), iCallback);
            } else {
                woyouService.sendRAWData(ESCUtil.underlineOff(), iCallback);
            }

            woyouService.printTextWithFont(content, null, size, iCallback);
    ...
    public void printTextWithTrans(List<PrintItemObj> list) throws RemoteException {
        ...
        woyouService.commitPrinterBufferWithCallback(null);

    }
    /**
     * 打印文字带回调
     */
    public void printTextWithTrans(List<PrintItemObj> list, ICallback iCallback) throws RemoteException {
        if (woyouService == null) {
            Toast.makeText(context, R.string.toast_2, Toast.LENGTH_LONG).show();
            return;
        }
        ...
        woyouService.commitPrinterBufferWithCallback(iCallback);
